...
- **Export to CSV.** An **Export CSV** button (in the summary strip) downloads the **current filtered offender list** for offline review and to feed `Invoke-MailboxCleanup.ps1` afterward. Uses the shared `ITTools.csv.download()` helper (consistent with Group Administration / Name Resolver). Filename `exchange-audit-offenders-YYYY-MM-DD.csv`. Columns: `UPN, DisplayName, RecoverableUsedGB, RecoverableQuotaGB, UsagePct, Status, ListingReason, SIR, MFAProcessing, RetentionHold, HoldsActive, DiscoveryHoldsGB, PurgesGB` (folder columns blank when `hasDetailStats` is false). **UPN is first** so a tech can lift the column straight into the cleanup workflow. Export reflects whatever filter/search is active, so "critical only" exports just those.
